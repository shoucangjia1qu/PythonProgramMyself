            ...
            if fext=='txt':
                ...
                for words in wordlist:
                    ...
                    for row in rows:
                        if words in row:
                            print(f)
                            print('【{}】在第{}行：“{}”'.format(words,n,row.strip()))
                            print('------------------------------------------')
                            m+=1
                        n+=1
            #对word文件搜索
            elif fext=='docx' or fext=='doc':
                #打开word
                
                doc=word.Documents.Open(f)    #新建文档
                for words in wordlist:
                    n=1
                    for p in doc.Paragraphs:
                        if words in p.Range.Text.strip():
                            print(f)
                            print('【{}】在第{}段：“{}”'.format(words,n,p.Range.Text.strip()))
                            print('------------------------------------------')
                            m+=1
                        n+=1
                doc.Close()
